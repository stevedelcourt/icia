      ...
      <main className="pt-36 pb-24">
        <div className="max-w-6xl mx-auto px-8">
          <nav className="flex gap-8 text-base text-gray-400 mb-16">
            {navActeurs.map((item) => (
              <Link key={item.href} href={item.href} className={`hover:text-black transition-colors duration-200 ${item.href === '/acteurs/education' ? 'text-black font-medium' : ''}`}>
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-start mb-24">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <h1 className="text-4xl md:text-5xl font-serif font-bold text-black leading-tight mb-6">Education</h1>
